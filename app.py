"""
================================================================================
  FASHION RETAIL PRICE OPTIMISER — DASHBOARD
  FYP Project · Sri Lanka Apparel Retail · Jan 2023 – Dec 2024
--------------------------------------------------------------------------------
  Methodology (from FYP-CODES.ipynb):
     Hybrid Demand Model
     ...
     • XGBoost predicts base demand at the CURRENT price using the product's
       ...
       attributes, calendar features). We do NOT swap prices inside the feature
       vector — that would create out-of-distribution rows.
     • Elasticity (log-log OLS, seasonality controls, clipped to [-8, 1.5])
       scales the XGBoost base demand whenever we consider a different price.
     • Candidate prices follow psychological "x90" anchors (90, 190, …, 990)
       within the business constraints defined in the notebook.

  Data files expected next to this script:
     result_df.csv           — per-product optimisation output (Phase 4)
     data_engineered.csv     — monthly engineered features per PATTERN
     model_metrics.json      — (optional) XGBoost R² / MAPE / RMSE
================================================================================
"""
...
# ================================================================
#  NOTEBOOK-ALIGNED CONSTANTS  (from Phase 4 — Price Simulation)
# ================================================================
PRICE_FLOOR_MULT   = 0.65      # Min price = current × 0.65
PRICE_CEIL_MULT    = 1.20      # Max price = current × 1.20
COST_MARGIN_MIN    = 1.05      # Min price also ≥ cost × 1.05
COST_MARGIN_MAX    = 2.00      # Max price also ≤ cost × 2.00
MIN_MARGIN_PCT     = 5.0       # Reject candidates with margin < 5%
QTY_DROP_GUARD_PCT = -30.0     # Reject candidates where demand collapses > 30%
PSYCH_ANCHORS      = [90, 190, 290, 390, 490, 590, 690, 790, 890, 990]
ELASTICITY_CLIP    = (-8.0, 1.5)   # Elasticity bounds used in notebook
...
# ================================================================
#  CUSTOM CSS  (dark / gold aesthetic — unchanged)
# ================================================================
st.markdown("""
<style>
...
@st.cache_data
def load_results():
    """Load result_df.csv — per-product optimisation output from the notebook."""
    ...
    for fname in ['result_df.csv', 'price_optimisation_results.csv']:
        ...
        if os.path.exists(path):
            ...
            # Back-compat: normalise any legacy column names → notebook names
            legacy_renames = {
            ...
            df.rename(columns={k: v for k, v in legacy_renames.items()
...
@st.cache_data
def load_engineered():
    """Load data_engineered.csv — monthly historical features per pattern."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
...
def load_model_metrics():
    """Load optional XGBoost performance metrics (if the notebook saved them)."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
...
def generate_candidate_prices(current_price, cost_price):
    """
    Mirror of `generate_candidate_prices()` in the notebook (Phase 4).
    Uses the SAME floor/ceil and psychological anchor rules.
    """
...
def hybrid_demand(candidate_price, current_price, xgb_base_demand, elasticity):
    """
    Mirror of `predict_demand_hybrid()` in the notebook.
        Final_Demand = XGBoost_Base × (candidate / current) ^ elasticity
    """
    if current_price <= 0 or xgb_base_demand <= 0:
        return 1.0
...
def check_constraints(candidate_price, cost_price, predicted_qty,
                      base_qty=None):
    """
    Mirror of `check_constraints()` in the notebook.
    Enforces min margin and (optionally) qty-drop guard.
    """
...
def compute_candidate_curve(current_price, cost_price, xgb_base_demand,
                            elasticity):
    """
    Build a candidate-price dataframe using the NOTEBOOK'S hybrid formula.
    Returns (df, best_row) where best_row is the profit-maximising feasible row.
    """
    ...
    for price in candidates:
        pred_qty   = hybrid_demand(price, current_price,
                                   xgb_base_demand, elasticity)
        revenue    = price * pred_qty
        profit     = (price - cost_price) * pred_qty
        margin_pct = (price - cost_price) / price * 100
        feasible   = check_constraints(price, cost_price, pred_qty,
                                       base_qty=xgb_base_demand)
...
def simulate_single_price(sim_price, current_price, cost_price,
                          xgb_base_demand, elasticity):
    """What-if on any manually chosen price using the hybrid formula."""
    pred_qty   = hybrid_demand(sim_price, current_price,
                               xgb_base_demand, elasticity)
    revenue    = sim_price * pred_qty
    profit     = (sim_price - cost_price) * pred_qty
    margin_pct = (sim_price - cost_price) / sim_price * 100 \
                 if sim_price > 0 else 0
    return pred_qty, revenue, profit, margin_pct


def get_product_history(pattern, eng_df):
    """Return sorted monthly history for a single pattern."""
    if eng_df is None:
        return pd.DataFrame()
...
def get_sale_velocity(history):
    """Compute velocity KPIs from monthly history."""
    if history.empty:
        return {'current_velocity': 0, 'trend_direction': 'No data',
                'trend_pct': 0, 'recent_avg': 0, 'older_avg': 0}
...
def get_similar_patterns(pattern, results_df, n=8):
    """FIT + MATERIAL match, falling back to FIT + COLOUR, then FIT only."""
...
with st.sidebar:
    ...
    cost_price = st.number_input(
        ...
        help="Used when exploring the Aggregate view. Per-product costs "
             "come straight from result_df.csv."
    )
    cost_range = st.slider(
        ...
        step=50,
        help="Filter products whose COST_PRICE falls in this range"
    )
    st.markdown("---")
    ...
    use_product_elasticity = st.checkbox(
        "Use per-product elasticity", value=True,
        help="Uses the ELASTICITY value stored in result_df.csv for the "
             "selected product. Disable to override with a manual value."
    )
    if not use_product_elasticity:
        manual_elasticity = st.slider(
            ...
            value=-1.5, step=0.1,
            help=f"Notebook clips elasticity to [{ELASTICITY_CLIP[0]}, "
                 f"{ELASTICITY_CLIP[1]}]. Values more negative ⇒ demand "
                 f"more sensitive to price."
        )
    else:
        manual_elasticity = -1.5
...
<div style='margin-bottom: 20px;'>
    ...
    <div class='main-subtitle'>
        Hybrid XGBoost + Elasticity · Sri Lanka Apparel Retail (Jan 2023 – Dec 2024)
    </div>
</div>
""", unsafe_allow_html=True)

# Methodology panel — explains the hybrid approach to the examiner
st.markdown("""
<div class='method-box'>
    <div class='title'>⚙ Methodology — Hybrid Demand Model</div>
    ...
    <div class='caption'>
        ...
        attributes, calendar). The elasticity <em>ε</em> (log-log OLS with
        seasonality controls, clipped to [-8, 1.5]) then scales that base
        whenever a different price is evaluated. The optimiser picks the
        psychological-anchor price that maximises profit, subject to min 5%
        margin and a 30% demand-drop guardrail.
    </div>
</div>
""", unsafe_allow_html=True)

# Optional model-performance strip
if model_metrics:
...
if results_df is None:
    ...
    <div class='info-box'>
        ...
        Place <code>result_df.csv</code> in the same directory and restart.
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
with col_left:
    if selected_filters:
        ...
        chips += f"<span class='tag' style='border-color:#444;color:#888;'>" \
...
with tab1:
    ...
    if not st.session_state.show_results or filtered_df.empty:
        ...
        # ── Product selector when multiple products match ───────
        ...
        if n_matched > 1:
            st.markdown(
                ...
                f"📋 {n_matched:,} products matched — select one to inspect its model-optimised price"
                f"</span></div>",
                unsafe_allow_html=True
            )
            ...
            _use_single = True
            _selected_product = filtered_df['PATTERN'].astype(str).iloc[0] \
                                 if n_matched == 1 else None

        # ── Resolve the row / derived inputs ────────────────────
        ...
        if _product_row is not None:
            # Single-product path — pull straight from result_df.csv
            current_price  = float(_product_row.get('CURRENT_PRICE',
                                                    cost_price * 2.0))
            effective_cost = float(_product_row.get('COST_PRICE', cost_price))

            # Prefer XGBoost base; fall back to historical avg monthly qty
            ...
            # Elasticity: notebook value unless overridden
            raw_elas = _product_row.get('ELASTICITY', None)
            if use_product_elasticity and raw_elas is not None and not pd.isna(raw_elas):
                elas = float(np.clip(raw_elas, *ELASTICITY_CLIP))
            else:
                elas = manual_elasticity

            # Model-chosen optimal (authoritative)
            ...
            # Aggregate path — representative scenario using medians
            current_price  = filtered_df['CURRENT_PRICE'].median() \
                             if 'CURRENT_PRICE' in filtered_df.columns \
                             else cost_price * 2.0
            effective_cost = filtered_df['COST_PRICE'].median() \
                             if 'COST_PRICE' in filtered_df.columns \
                             else cost_price
            xgb_base = filtered_df['XGB_BASE_DEMAND'].median() \
                       if 'XGB_BASE_DEMAND' in filtered_df.columns \
                       else filtered_df.get('CURRENT_QTY', pd.Series([10.0])).median()
            xgb_base = float(xgb_base) if not pd.isna(xgb_base) else 10.0

            if use_product_elasticity and 'ELASTICITY' in filtered_df.columns:
                elas = float(np.clip(filtered_df['ELASTICITY'].dropna().median(),
                                     *ELASTICITY_CLIP))
                if pd.isna(elas):
                    elas = manual_elasticity
            else:
            ...
            # Aggregate: run the hybrid optimiser on the median scenario
            ...
            price_change_pct = (model_optimal_price - current_price) \
                               / current_price * 100 if current_price > 0 else 0
            recommendation   = ('INCREASE PRICE' if model_optimal_price > current_price
        ...
        optimal_margin = (model_optimal_price - effective_cost) \
                         / model_optimal_price * 100 \
                         if model_optimal_price and model_optimal_price > 0 else 0
        ...
        with m2:
            ref_label = "Current Price" if _product_row is not None else "Median Current Price"
            st.metric(ref_label, f"Rs. {current_price:,.0f}")
        with m3:
            st.metric("XGBoost Base Demand", f"{xgb_base:.0f} units",
                      help="XGBoost's predicted monthly demand at the current price")
        with m4:
            e_help = f"Product's own elasticity (clipped to [{ELASTICITY_CLIP[0]}, {ELASTICITY_CLIP[1]}])" \
                     if use_product_elasticity else "Manual override"
            st.metric("Elasticity (ε)", f"{elas:.2f}", help=e_help)
        with m5:
            count_label = ("1 Product" if _product_row is not None and n_matched > 1
                           else f"{n_matched:,} Products")
            st.metric("Viewing", count_label)

        st.markdown("<br>", unsafe_allow_html=True)

        # ── Optimal price badge + Revenue/Profit curve ─────────
        opt_col, chart_col = st.columns([1, 2])

        # Compute the candidate curve (for the chart + simulator)
        cand_df, _best_row = compute_candidate_curve(
            current_price, effective_cost, xgb_base, elas
        )

        with opt_col:
            rec_color = ('#5cb85c' if recommendation == 'INCREASE PRICE'
        ...
        with chart_col:
            ...
            # Use only feasible candidates for the main curves (others shown ghosted)
            ...
            if not infeasible_df.empty:
                fig.add_trace(go.Scatter(
                    ...
                    hovertemplate='Price: Rs. %{x:,}<br>Profit: Rs. %{y:,.0f}<br>'
            ...
            for col_name in ['Revenue', 'Profit']:
                ...
                if not opt_val.empty:
                    fig.add_trace(go.Scatter(
                        x=[model_optimal_price], y=[opt_val.values[0]],
                        mode='markers',
                        name=f'Optimal ({col_name})',
                        marker=dict(size=13, color='#ff6b6b',
                                    symbol='star',
                                    line=dict(color='white', width=1.5)),
                        showlegend=(col_name == 'Revenue'),
                        hovertemplate=(
            ...
            fig.add_vline(
                x=current_price, line_dash='dash', line_color='#555',
                annotation_text=f'Current: Rs. {int(current_price):,}',
                annotation_font_color='#888', annotation_font_size=10
            )

            layout = dark_chart_layout('Hybrid Revenue & Profit Curves', 380)
        ...
        # ── LIVE PRICE SIMULATOR (hybrid formula) ──────────────
        st.markdown(
            "<div class='section-header'>🎛️  Live Price Simulator — Hybrid Formula</div>",
            unsafe_allow_html=True
        )
        st.markdown(
            "<div style='color:#666;font-size:0.85rem;margin-bottom:16px;'>"
            "Drag the slider to see what any price produces under the "
            "<code>XGB_base × (new/current)^ε</code> hybrid model — and how it "
            "compares to the model's profit-maximising choice.</div>",
            unsafe_allow_html=True
        )
        ...
        sim_default = model_optimal_price if st.session_state.sim_price is None \
                      else st.session_state.sim_price
        sim_default = int(max(sim_min, min(sim_max, sim_default)))

        sim_price = st.slider(
            "Simulated Price (Rs.)",
            min_value=sim_min, max_value=sim_max,
            value=sim_default, step=100,
            key='sim_slider'
        )
        ...
        # Feasibility flag for the simulated price
        ...
        # ── Candidate Prices Table ─────────────────────────────
        st.markdown(
            "<div class='section-header'>All Candidate Prices (Hybrid)</div>",
            unsafe_allow_html=True
        )

        display_cands = cand_df.copy()
        display_cands.insert(0, '', display_cands['Candidate Price'].apply(
            lambda x: '⭐ OPTIMAL' if x == model_optimal_price
                      else ('🎛️ SIM'   if x == sim_price else '')
        ))
        display_cands['Feasible'] = display_cands['Feasible'].map(
            {True: '✓', False: '✕'}
        )
        display_cands['Candidate Price'] = display_cands['Candidate Price'] \
            .apply(lambda x: f'Rs. {int(x):,}')
        display_cands['Predicted QTY']   = display_cands['Predicted QTY'] \
...
with tab2:
    ...
    if filtered_df.empty:
        ...
        avg_current  = filtered_df['CURRENT_PRICE'].mean() \
                       if 'CURRENT_PRICE' in filtered_df.columns else 0
        avg_optimal  = filtered_df['OPTIMAL_PRICE'].mean() \
                       if 'OPTIMAL_PRICE' in filtered_df.columns else 0
        avg_rev_imp  = filtered_df['REVENUE_IMPROVEMENT_%'].mean() \
                       if 'REVENUE_IMPROVEMENT_%' in filtered_df.columns else 0
        avg_prof_imp = filtered_df['PROFIT_IMPROVEMENT_%'].mean() \
                       if 'PROFIT_IMPROVEMENT_%' in filtered_df.columns else 0
        pct_success  = ((filtered_df['OPTIMIZATION_STATUS'] == 'SUCCESS').mean()
                        * 100) if 'OPTIMIZATION_STATUS' in filtered_df.columns else 0
        ...
        with p5: st.metric("Repriced",           f"{pct_success:.0f}%",
                           help="% of products where a better price was found")

        # ── Portfolio financial impact strip ───────────────────
        ...
        # ── Row 1: Price-change hist + Recommendation pie ──────
        ...
        with row1_r:
            if 'RECOMMENDATION' in filtered_df.columns:
                ...
                # Notebook uses UPPERCASE labels
                color_map = {
                    'INCREASE PRICE': '#5cb85c',
                    'MAINTAIN PRICE': '#c8a96e',
                    'DECREASE PRICE': '#e05c5c',
                }
                colors_pie = [color_map.get(l, '#888') for l in rec_counts.index]
        ...
        # ── Row 2: Elasticity hist + Profit scatter ────────────
        ...
        with row2_l:
            if 'ELASTICITY' in filtered_df.columns:
                ...
                fig_elas.add_vline(x=-1, line_dash='dash', line_color='#e05c5c',
        ...
        with row2_r:
            if all(c in filtered_df.columns
                ...
                fig_scatter = go.Figure(go.Scatter(
                    ...
                    text=filtered_df.get('PATTERN',
                                         pd.Series([''] * len(filtered_df)))
                ))
                fig_scatter.add_hline(y=0, line_dash='dash', line_color='#555')
                layout_s = dark_chart_layout(
                    'Profit Improvement vs Current Price', 320
                )
        ...
        # ── Row 3: XGBoost base demand vs actual monthly qty ──
        if all(c in filtered_df.columns
               for c in ['XGB_BASE_DEMAND', 'CURRENT_QTY']):
            st.markdown("<div class='custom-divider'></div>",
                        unsafe_allow_html=True)
            max_val = max(filtered_df['XGB_BASE_DEMAND'].max(),
            ...
            fig_parity.add_trace(go.Scatter(
                ...
                mode='lines', line=dict(color='#555', dash='dash'),
                name='Parity (y=x)', hoverinfo='skip'
            ))
    ...
    if filtered_df.empty:
        ...
        with sort_col:
            sort_options = [c for c in [
                'PROFIT_IMPROVEMENT_$', 'REVENUE_IMPROVEMENT_%',
        ...
        if 'PATTERN' in explorer_df.columns and len(explorer_df) > 0:
            ...
            with dd1:
                ...
                for attr in ['COLOUR', 'FIT', 'MATERIAL', 'SLEEVE',
                             'COLLAR', 'TEXTURE']:
            ...
            with dd3:
                for label, col, fmt in [
                    ...
                    ("Hybrid Predicted Qty", 'PREDICTED_QTY_HYBRID', "{:.1f} units"),
        ...
        if engineered_df is not None and selected_pattern:
            ...
            if not history.empty:
                ...
                with vk1: st.metric("Total Months", f"{len(history)}")
                with vk2: st.metric("Total Qty Sold",
                                    f"{int(history['QTY'].sum()):,}")
                with vk3: st.metric("Avg Monthly Qty",
                                    f"{history['QTY'].mean():.1f}")
                with vk4:
                    direction = velocity['trend_direction']
                    icon = ('↑' if direction == 'Accelerating'
                            else '↓' if direction == 'Decelerating' else '→')
                    dir_color = ('#5cb85c' if direction == 'Accelerating'
                ...
                with vk5:
                    st.metric(
                        ...
                        delta=f"Recent 3M: {velocity['recent_avg']:.1f} "
                ...
                # Monthly qty bar chart + rolling means
                fig_hist = go.Figure()
                overall_mean = history['QTY'].mean()
                bar_colors = ['#5cb85c' if q >= overall_mean else '#e05c5c'
                ...
                if len(history) >= 3:
                    ...
                    fig_vel.add_trace(go.Scatter(
                        x=history_v['YEAR_MONTH'].dt.strftime('%Y-%m'),
                        y=trend_line, mode='lines', name='Sales Trend',
                        line=dict(color='#ff6b6b' if z[0] < 0 else '#5cb85c',
                                  width=2.5, dash='dot'),
            ...
            if similar_df.empty:
                ...
                ref_row = results_df[
                    results_df['PATTERN'] == str(selected_pattern)
                ]
                sim_basis = ''
                if not ref_row.empty:
                    r = ref_row.iloc[0]
                    sim_basis = (f"FIT: <b>{r.get('FIT','?')}</b> &nbsp;·&nbsp; "
                                 f"MATERIAL: <b>{r.get('MATERIAL','?')}</b>")
                st.markdown(
                    f"<div style='color:#888;font-size:0.83rem;"
                    f"margin-bottom:16px;'>Matched by {sim_basis}</div>",
                    unsafe_allow_html=True
                )
                ...
                compare_df = pd.concat([current_row, similar_df],
                                       ignore_index=True) \
                ...
                with sc1:
                    if 'CURRENT_QTY' in compare_df.columns:
                        comp_sorted = compare_df.sort_values(
                            'CURRENT_QTY', ascending=True
                        )
                        bar_colors_s = [
                            '#c8a96e' if str(p) == str(selected_pattern)
                            else '#4a4a6a'
                        ...
                        fig_cmp = go.Figure(go.Bar(
                            ...
                            marker=dict(color=bar_colors_s,
                                        line=dict(color='#0f0f0f', width=0.3)),
                            hovertemplate=('Pattern: %{y}<br>'
                                           'Avg Monthly Qty: %{x:,}<extra></extra>'),
                            text=comp_sorted['CURRENT_QTY'].apply(
                                lambda x: f'{int(x):,}'
                            ),
                ...
                with sc2:
                    ...
                    for pat in all_patterns:
                        ...
                        fig_multi.add_trace(go.Scatter(
                            ...
                            hovertemplate=(f'Pattern {pat}<br>'
                                           'Month: %{x}<br>'
                                           'Qty: %{y:,}<extra></extra>')
                        ))
                ...
                for pat in compare_df['PATTERN'].astype(str):
                    ...
                    vel_rows.append({
                        'Pattern':         ('★ ' + pat
                                            if pat == str(selected_pattern)
                                            else pat),
                        'Avg Monthly Qty': (round(pat_hist['QTY'].mean(), 1)
                                            if not pat_hist.empty else '-'),
                        'Last Month Qty':  (int(pat_hist['QTY'].iloc[-1])
                                            if not pat_hist.empty else '-'),
                        'Trend':           v['trend_direction'],
                        'Velocity Change': f"{v['trend_pct']:+.1f}%",
                        'Months of Data':  len(pat_hist),
                        'Optimal Price':   (f"Rs. {int(pat_info['OPTIMAL_PRICE'].iloc[0]):,}"
                                            if 'OPTIMAL_PRICE' in pat_info.columns
                                            and not pat_info.empty else '-'),
                    })

                vel_df = pd.DataFrame(vel_rows)
                st.dataframe(
                    vel_df, use_container_width=True,
                    hide_index=True,
...
<div style='text-align:center; color:#444; font-size:0.75rem; padding:8px 0 20px 0;'>
    ...
    &nbsp;·&nbsp; Hybrid XGBoost + Log-Log Elasticity
</div>
""", unsafe_allow_html=True)
